...
def boar_brawl(player_score, opponent_score):
    ...
    ones = (player_score + 10) % 10
    bb_score = abs(tens - ones) * 3
    if bb_score == 0:
        bb_score = 1
    return bb_score
    # END PROBLEM 2
...
def num_factors(n):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    assert type(n) == int, 'num_factors must be an integer.'
    assert n > 0, 'num_factors must be positive.'
    i = 1
    num_factors_sum = 0
    for i in range(1,n+1):
        if n % i == 0:
            num_factors_sum = num_factors_sum + 1
    return num_factors_sum
    # END PROBLEM 4
...
def play(strategy0, strategy1, update,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            ...
            num_rolls = int(strategy1(score1,score0))
...
def is_always_roll(strategy, goal=GOAL):
    ...
    score0 = 0
    score1 = 0
    starter_number_of_rolls = strategy(0,0)
    for score0 in range(goal):
        for score1 in range(goal):
            if strategy(score0,score1) != starter_number_of_rolls:
                return False
    return True
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    max_scoring_num = 1
    for i in range(1,11):
        result = make_averaged(roll_dice, samples_count)(i,dice)
